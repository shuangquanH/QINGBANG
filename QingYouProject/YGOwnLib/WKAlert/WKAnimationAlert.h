...
typedef enum {
    ...
    WKAlertAnimationTypeCustom
} WKAlertAnimationType;

@interface WKAnimationAlert : UIView

+ (void)dismissAnimation:(void(^)(UIView *insideView))animation;
...
+ (void)showAlertWithInsideView:(UIView *)insideView canTouchDissmiss:(BOOL)canTouchDissmiss superView:(UIView *)superView animation:(void(^)(UIView *insideView))animation dismiss:(void(^)(UIView *insideView))dismiss;
