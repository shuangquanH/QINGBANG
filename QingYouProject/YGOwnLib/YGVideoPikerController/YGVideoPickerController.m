...
-(void)configCamera
{
    ...
    //设置图层的大小
    preLayer.frame = CGRectMake(0, 44, self.view.frame.size.width, self.view.frame.size.width *(640.0/480.0));
    //添加到view上
...
-(void)configCameraUI
{
    ...
    [_videoButton setBackgroundImage:[UIImage imageNamed:@"xianzhi_luzhi_dianji.png"] forState:UIControlStateNormal];
    [_videoButton addTarget:self action:@selector(videoButtonTouchDown) forControlEvents:UIControlEventTouchDown];
    [_videoButton addTarget:self action:@selector(videoButtonTouchUpInside) forControlEvents:UIControlEventTouchUpInside];
    [_videoButton addTarget:self action:@selector(videoButtonTouchDragExit) forControlEvents:UIControlEventTouchDragExit];
    [_videoButton sizeToFit];
    _videoButton.center = CGPointMake(self.view.frame.size.width/2, (self.view.frame.size.height - _videoProgressView.frame.origin.y - _videoProgressView.frame.size.height) / 2 + _videoProgressView.frame.origin.y + _videoProgressView.frame.size.height);
    ...
    xButton.center =  CGPointMake(xButton.center.x, _videoButton.center.y);
...
//录像键上滑
-(void)videoButtonTouchDragExit
{
    _stateLabel.text = @"取消了录制";
    [_movieFileOutPut stopRecording];
    //取消了就删除掉
    _enoughTime = NO;
    [self formatCamera];
