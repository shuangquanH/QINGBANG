...
#import "LDNetManagerApplyVC.h"//申请信息:第一个是所在园区
#import "LDTextView.h"//换行TextView
#import "HXTagsView.h"//TagView
#import "LDPayViewController.h"//立即支付,顶部月卡
#import <WebKit/WebKit.h>
#import "AreaSelectViewController.h"

@interface LDNetManagerApplyVC ()<WKNavigationDelegate>
/** 背景ScrollowView  */
...
@implementation LDNetManagerApplyVC

- (void)viewDidLoad {
    [super viewDidLoad];
    //设置导航栏
    self.naviTitle = @"VIP包月网络管理服务";
    //设置UI视图
    [self setupUI];
    //网络请求
    [self sendRequest];
...
- (void)bottomButtonClick:(UIButton *)bottomButton{
    ...
    LDPayViewController * VC= [[LDPayViewController alloc] init];
...
- (void)sendRequest{
    ...
    [YGNetService YGPOST:@"NetVIPApply" parameters:@{@"isVIP":self.isVIP} showLoadingView:YES scrollView:nil success:^(id responseObject) {
