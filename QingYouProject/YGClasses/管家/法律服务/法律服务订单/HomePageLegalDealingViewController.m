//
//  HomePageLegalDealingViewController.m
//  QingYouProject
//
...
#import "HomePageLegalDealingViewController.h"
#import "WaitToNetManagerTableViewCell.h"
#import "WaitToNetManagerModel.h"
@interface HomePageLegalDealingViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HomePageLegalDealingViewController
                 ...
                 success:^( id responseObject) {
                     ...
                     else
                     {
                         if([self.pushView isEqualToString:@"isVIP"])
                         {
                             ...
                             if ([responseObject[@"lawVipOrder"] count] == 0)
                             {
                                 //调用一下没数据的方法，告诉用户没有更多
                     ...
                     if([self.pushView isEqualToString:@"isVIP"])
                     {
                         [self.dataArray addObjectsFromArray:[WaitToNetManagerModel mj_objectArrayWithKeyValuesArray:responseObject[@"lawVipOrder"]]];
                     }
                     else
...
{
    UIView *footView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, YGScreenWidth, 45)];
    footView.backgroundColor = [UIColor whiteColor];
    
    UILabel * lineTwo = [[UILabel alloc]initWithFrame:CGRectMake(0,0, YGScreenWidth, 1)];
    lineTwo.backgroundColor = colorWithTable;
    [footView addSubview:lineTwo];
    
    UILabel *processingLabel = [[UILabel alloc]initWithFrame:CGRectMake(LDHPadding, 0, YGScreenWidth - LDHPadding*2, footView.height)];
