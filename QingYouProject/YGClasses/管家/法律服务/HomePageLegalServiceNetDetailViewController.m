//
//  HomePageLegalServiceNetDetailViewController.m
//  QingYouProject
//
...
#import "HomePageLegalServiceLDApplyVCViewController.h"//立即申请
...
@interface HomePageLegalServiceNetDetailViewController ()<UIScrollViewDelegate,SDCycleScrollViewDelegate,YGSegmentViewDelegate,HomePageLegalServiceIntroduceViewControllerDelegate,ServiceEvaluationViewControllerDelegate,TradeRecordViewControllerDelegate>
{
    NSMutableArray * _controllersArray;//Controller数组
    YGSegmentView * _segmentView;//选择器
    UIScrollView * _scrollView;
    HomePageLegalServiceIntroduceViewController * _netIntroduceViewController;
...
@implementation HomePageLegalServiceNetDetailViewController
{
...
{
    NSLog(@"self.serviceID ---  %@  ,,   YGSingletonMarco.user.userId --  %@ ",self.serviceID,YGSingletonMarco.user.userId);
    
    [YGNetService YGPOST:@"LawService" parameters:@{@"serviceID":self.serviceID,@"userID":YGSingletonMarco.user.userId} showLoadingView:YES scrollView:nil success:^(id responseObject) {
...
-(void)loadControllerWithIndex:(int)index
{
    if (index == 0)
    {
        if (_netIntroduceViewController == nil) {
            _netIntroduceViewController = [[HomePageLegalServiceIntroduceViewController alloc]init];
    ...
    else if (index == 1)
    {
        if (_serviceEvaluationViewController == nil) {
            ...
            _serviceEvaluationViewController.superVCType = @"HomePageLegal";
    ...
    }else
    {
        if (_tradeRecordViewController == nil) {
            ...
            _tradeRecordViewController.superVCType = @"HomePageLegal";
...
- (void)contanctWithCustomerServiceOrOrderAction:(UIButton *)btn
{
    if (btn.tag == 1000)
    {//联系客服
        [self contactWithCustomerServerWithType:ContactServerIndoorCommonCall button:btn];
    }else{//立即申请
        
        HomePageLegalServiceLDApplyVCViewController * applyVC = [[HomePageLegalServiceLDApplyVCViewController alloc] init];
...
- (void)favoriteButtonAction:(UIButton *)btn
{
    ...
    [YGNetService YGPOST:@"LawServiceCollect" parameters:dict showLoadingView:YES scrollView:nil success:^(id responseObject) {
