//
//  HomePageLegalServiceViewController.m
//  QingYouProject
//
...
#import "HomePageLegalServiceManagerDetailViewController.h"//服务介绍
#import "HomePageLegalServiceVIPDetailViewController.h"//VIP卡介绍
#import "HomePageLegalServiceManagerApplyViewController.h"//VIP网络管理服务,立即申请页面
#import "HomePageLegalServiceNetDetailViewController.h"//商品详情
#import "NetVIPmodel.h"


@interface HomePageLegalServiceViewController ()<UITableViewDelegate,SDCycleScrollViewDelegate,UITableViewDataSource>
/** 轮播图  */
...
@implementation HomePageLegalServiceViewController
...
- (void)setupNav{
    ...
    // 右边按钮
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemWithimage:[UIImage imageNamed:@"service_black"] highImage:[UIImage imageNamed:@"service_green"] target:self action:@selector(rightBarButtonClick:)];
    
}
#pragma mark - 导航栏右侧按钮点击
- (void)rightBarButtonClick:(UIButton *)rightBarButton{
    
    [self contactWithCustomerServerWithType:ContactServerIndoorCommonCall button:rightBarButton];
}
...
- (void)sendRequest{
    ...
    [YGNetService YGPOST:@"LawIndex" parameters:@{@"userID":YGSingletonMarco.user.userId} showLoadingView:YES scrollView:nil success:^(id responseObject) {
        ...
        for (UIButton * subView in self.footerView.subviews)
        {
            switch (subView.tag) {
                case 100:
                    [subView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",responseObject[@"imgLawTransaction"]]] forState:UIControlStateNormal placeholderImage:YGDefaultImgTwo_One];
                    break;
                case 101:
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    {
        [YGNetService YGPOST:@"IndoorCall" parameters:@{@"rank":@"17"} showLoadingView:NO scrollView:nil success:^(id responseObject) {
...
- (void)bottomButtonClick:(UIButton *)bottomButton{
    ...
    switch (tag) {
        ...
        {
            //网络管理服务介绍
            HomePageLegalServiceManagerDetailViewController * vc = [[HomePageLegalServiceManagerDetailViewController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
        }
            break;
        case 101:
        {
            //VIP介绍
            HomePageLegalServiceVIPDetailViewController * vc = [[HomePageLegalServiceVIPDetailViewController alloc] init];
            vc.isVIP =self.isVIP;
            [self.navigationController pushViewController:vc animated:YES];
        }
            break;
