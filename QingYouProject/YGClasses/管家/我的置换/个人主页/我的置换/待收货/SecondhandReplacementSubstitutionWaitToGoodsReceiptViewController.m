//
//  SecondhandReplacementSubstitutionWaitToGoodsReceiptViewController.m
//  QingYouProject
//
...
#import "SecondhandReplacementSubstitutionWaitToGoodsReceiptViewController.h"
...
@interface SecondhandReplacementSubstitutionWaitToGoodsReceiptViewController ()<UITableViewDelegate,UITableViewDataSource,SecondhandReplacementSubstitutionWaitToGoodsTableViewCellDelegate>
{
...
@implementation SecondhandReplacementSubstitutionWaitToGoodsReceiptViewController
...
- (void)viewDidLoad {
    ...
    _segmentedControl = [[UISegmentedControl alloc]initWithItems:@[@"待我收货",@"待对方收货"]];
...
{
    [YGAlertView showAlertWithTitle:@"双方确认收货后将解冻保证金，\n是否确认收货？" buttonTitlesArray:@[@"取消",@"确定"] buttonColorsArray:@[colorWithBlack,colorWithMainColor] handler:^(NSInteger buttonIndex) {
        if (buttonIndex == 0) {
            return ;
        }else
        {
            ...
            [YGNetService YGPOST:@"confirmReceipt" parameters:dict showLoadingView:YES scrollView:nil success:^(id responseObject) {
                ...
                [YGAppTool showToastWithText:@"收货成功"];
...
- (void)secondhandReplacementSubstitutionWaitToGoodsTableViewCellCustomerServiceButton:(UIButton *)btn withRow:(NSInteger)row
{
...
{
    SecondhandReplacementIBoughtModel * model;
    if(_isOther)
        model = _otherDataArray[row];
    else
...
-(void)createAddressViewwithTitle:(NSString *)title withRow:(NSInteger )row
{
    ...
    SecondhandReplacementIBoughtModel * model = _otherDataArray[row];
