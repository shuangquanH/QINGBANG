...
@implementation RushPurchaseDetailPopView

{
    ...
    RushPurchaseCommodityModel *_firstModel;
...
- (void)createFrame:(CGRect)frame withInfoNSArry:(NSArray *)arry
{
    ...
    cancleButton.titleLabel.font = [UIFont systemFontOfSize:YGFontSizeBigOne];
    ...
    [_leftImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_firstModel.img]] placeholderImage:YGDefaultImgSquare];
    
    //新鲜事标题label
    _titleLabel= [[UILabel alloc]init];
    _titleLabel.textColor = colorWithOrangeColor;
    _titleLabel.font = [UIFont systemFontOfSize:YGFontSizeBigFour];
    _titleLabel.text = [NSString stringWithFormat:@"¥%@",_firstModel.newprice];
    ...
    _describeLabel.text = [NSString stringWithFormat:@"库存  %@",_firstModel.stock];;
...
- (CGSize)createLabelsWithbaseView:(UIView *)baseView
{
    ...
    for (int i = 0;i<_labelList.count;i++) {
        ...
        button.frame = CGRectMake(widthCount+k*10, 30*j, labeWidth, 25) ;
        
        widthCount = widthCount +labeWidth;
        
        if (widthCount>(baseView.width-10-k*10)) {
            j=j+1;
            k=0;
            widthCount = 0.0f;
            button.frame = CGRectMake(widthCount+k*10,30*j, labeWidth, 25) ;
        ...
        if (model.select == YES)
        {
            ...
            _describeLabel.text = [NSString stringWithFormat:@"库存  %d",[model.stock intValue]-[_firstModel.number intValue]];
            _yearLabel.text = [NSString stringWithFormat:@"%d",[_firstModel.number intValue]];
        }
...
- (void)buttonClick:(UIButton *)btn{
    ...
    RushPurchaseCommodityModel *model = _labelList[_selectNum];
    model.select = YES;
   _firstModel = _labelList[_selectNum];

    [_leftImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.img]] placeholderImage:YGDefaultImgSquare];
    _titleLabel.text = [NSString stringWithFormat:@"¥%@",model.newprice];
    _describeLabel.text = [NSString stringWithFormat:@"库存  %d",[model.stock intValue]];
...
- (void)addButtonAction:(UIButton *)btn
{
    ...
    if ([_yearLabel.text intValue] >= [_firstModel.stock intValue]) {
        [YGAppTool showToastWithText:@"剩余商品不够啦！"];
        return ;
    }
    _yearLabel.text = [NSString stringWithFormat:@"%d",[_yearLabel.text intValue]+1];
    _firstModel.number = _yearLabel.text;
    _leftNumber = [NSString stringWithFormat:@"%d",[_firstModel.stock intValue]-[_yearLabel.text intValue]];
    _describeLabel.text = [NSString stringWithFormat:@"库存  %@",_leftNumber];
...
- (void)subButtonAction:(UIButton *)btn
{
    ...
    _yearLabel.text = [NSString stringWithFormat:@"%d",[_yearLabel.text intValue]-1];
    _firstModel.number = _yearLabel.text;
    _leftNumber = [NSString stringWithFormat:@"%d",[_leftNumber intValue]+1];
    _describeLabel.text = [NSString stringWithFormat:@"库存  %@",_leftNumber];
...
- (void)nextStepButtonAction:(UIButton *)btn
{
    ...
    [self.delegate chooseGoodsWithCategoriesViewSurePaywithCommoditySizeId:model.id andCommoditySizeSum:_yearLabel.text andRushPurchaseCommodityModel:model];
    [self selfDisappear];
}
