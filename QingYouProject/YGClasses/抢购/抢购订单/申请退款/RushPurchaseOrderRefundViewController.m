//
//  RushPurchaseOrderRefundViewController.m
//  QingYouProject
//
...
@interface RushPurchaseOrderRefundViewController ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UITextViewDelegate>
{
...
@implementation RushPurchaseOrderRefundViewController
...
//    [self.goodsDetailView reloadDataWithImage:self.model.commodityImg name:self.model.commodityName rule:[NSString stringWithFormat:@"%@",self.model.commoditySizeName] price:self.model.unitPrice count:[NSString stringWithFormat:@"x%@",self.model.commodityNum]];
    [self.goodsDetailView pushPurchaseReloadDataWithImage:_model.commodityImg name:_model.commodityName price:[NSString stringWithFormat:@"¥%@",_model.unitPrice] count:_model.commoditySizeName];

    self.describe.text = [NSString stringWithFormat:@"最多能退回¥%@元，含发货邮费¥%@元",self.model.totalPrice,self.model.freight];
...
- (void)rightBarButtonClick:(UIButton *)rightBarButton{
    ...
    {
        [YGAppTool showToastWithText:@"请填写退款原因！"];
        return;
    }
    ...
    [UploadImageTool uploadImages:_selectedPhotos progress:^(CGFloat progress) {
        ...
        NSDictionary * parameters = @{@"falshsaleOrderId":_model.falshsaleOrderId,
                                      @"cause":self.detailTextView.text,
                                      @"refundsImg":urlStr,
                                      };
        
        [YGNetService YGPOST:REQUEST_outFalshsaleOrder parameters:parameters showLoadingView:YES scrollView:nil success:^(id responseObject) {
            
            [YGAppTool showToastWithText:@"提交成功"];
            if([self.isPush isEqualToString:@"list"])
            {
                [self.delegate RushPurchaseOrderRefundViewControllerWithCommintRow:self.row];
