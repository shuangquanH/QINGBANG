//
//  MineProjectApplyWaitCheckViewController.m
//  QingYouProject
//
...
@interface MineProjectApplyWaitCheckViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation MineProjectApplyWaitCheckViewController
...
- (void)refreshActionWithIsRefreshHeaderAction:(BOOL)headerAction
{
    ...
    [YGNetService YGPOST:REQUEST_SearchApplicationOrder parameters:@{@"userId":YGSingletonMarco.user.userId,@"auditStatus":@"1",@"total":self.totalString,@"count":self.countString} showLoadingView:NO scrollView:_tableView success:^(id responseObject) {
        if (headerAction == YES) {
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    [deleteButton setTitle:@"取消申请" forState:UIControlStateNormal];
    ...
    deleteButton.frame = CGRectMake(YGScreenWidth-deleteButton.width-30,7,deleteButton.width+20,30);;
...
- (void)deliverCurriculumButtonAction:(UIButton *)btn
{
    [YGAlertView showAlertWithTitle:@"您确定要取消这条申请吗？" buttonTitlesArray:@[@"取消",@"确定"] buttonColorsArray:@[colorWithBlack,colorWithMainColor] handler:^(NSInteger buttonIndex) {
        if (buttonIndex == 1)
        {
            MineProjectApplyModel *model = _listArray[btn.tag-1000];
