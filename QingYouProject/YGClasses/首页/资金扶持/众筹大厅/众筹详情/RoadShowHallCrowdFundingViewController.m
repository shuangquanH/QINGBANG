//
//  RoadShowHallCrowdFundingViewController.m
//  QingYouProject
//
...
@interface RoadShowHallCrowdFundingViewController ()<UIScrollViewDelegate,SDCycleScrollViewDelegate,YGSegmentViewDelegate>
{
...
@implementation RoadShowHallCrowdFundingViewController
{
...
{
    self.naviTitle = @"项目详情";
   UIBarButtonItem *shareButtonItem = [self createBarbuttonWithNormalImageName:@"share_black" selectedImageName:@"share_black" selector:@selector(shareButtonAction)];
...
-(void)loadData
{
    [self startPostWithURLString:REQUEST_getProjectDetails parameters:@{@"id":_projectID,@"uId":YGSingletonMarco.user.userId} showLoadingView:NO scrollView:nil];
}
...
{
    [_model setValuesForKeysWithDictionary:responseObject[@"projectDetails"]];
    _model.shutFlag = responseObject[@"shutFlag"];
    _model.url = responseObject[@"url"];
    _model.colFlag = responseObject[@"colFlag"];
    _userNameString = responseObject[@"usmUser"][@"name"];
    _avaterImageUrl = responseObject[@"usmUser"][@"img"];
    _model.selFlag = responseObject[@"selFlag"];
    _model.subFlag = responseObject[@"subFlag"];
    _model.calmStr = responseObject[@"calmStr"];
...
-(void)configUI
{
    ...
    _mainScrollView.contentSize = CGSizeMake(YGScreenWidth, _mainScrollView.height+20);
    ...
    _titleLabel.frame = CGRectMake(10, _projectVideoImageView.height+10,YGScreenWidth-30, _titleLabel.height+10);
    ...
    _contenttionLabel.frame = CGRectMake(10,_titleLabel.y+_titleLabel.height+5, YGScreenWidth-20, _contenttionLabel.height+10);
    ...
    /********************** 进度条 众筹 ********************/
    UIView *progressBaseView = [[UIView alloc] initWithFrame:CGRectMake(10, incomeRightsImageView.y+incomeRightsImageView.height+10, YGScreenWidth-20, 60)];
    ...
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0,_baseView.y+_baseView.height, YGScreenWidth, 100)];
    ...
    //neirong
    _contentLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 10, YGScreenWidth-30, 20)];
    ...
    NSArray *btnTitleArr = @[@{@"title":@"交流群",@"img":@"steward_capital_talk"},@{@"title":@"收藏",@"img":@"collect_icon_black_little"}];
    for (int i = 0; i<3; i++)
    {
        ...
        if (i == 0 || i == 1)
        {
            coverButton.frame = CGRectMake(YGScreenWidth/4*i, 0, YGScreenWidth/4, 45+YGBottomMargin);
        ...
        {
            coverButton.frame = CGRectMake(YGScreenWidth/2, 0, YGScreenWidth/2, 45+YGBottomMargin);
...
//{
//    _mainScrollView.frame = CGRectMake(_mainScrollView.x, _scrollView.y, _scrollView.width, contentSize.height);
...
- (void)contanctOrFavoriteOrSubscribeBtnAction:(UIButton *)btn
{
    ...
    if (btn.tag == (1000+2))
    {
        ...
        if ([_model.selFlag isEqualToString:@"1"]) {
            [YGAppTool showToastWithText:@"自己发起的项目是不支持认购的哦"];
            return;
        }
        if ([_model.subFlag isEqualToString:@"1"]) {
            [YGAppTool showToastWithText:@"您已经认购过该项目"];
            return;
        }
        if ([_model.shutFlag isEqualToString:@"1"]) {
            [YGAppTool showToastWithText:@"该项目发起人已终止认购该项目"];
            return;
        }
        [YGNetService YGPOST:REQUEST_authentication parameters:@{@"userId":YGSingletonMarco.user.userId} showLoadingView:NO scrollView:nil success:^(id responseObject) {
            ...
            {
                RSCrowFundingSubscribeViewController *crowFundingSubscribeViewController = [[RSCrowFundingSubscribeViewController alloc] init];
                crowFundingSubscribeViewController.projectID = _projectID;
                crowFundingSubscribeViewController.projectName = _model.projectName;
                crowFundingSubscribeViewController.coolTimeDescription = _model.calmStr;
                [self.navigationController pushViewController:crowFundingSubscribeViewController animated:YES];
            }
    ...
    if (btn.tag == 1001)
    {
        [YGNetService YGPOST:REQUEST_proCollect parameters:@{@"pId":_projectID,@"uId":YGSingletonMarco.user.userId} showLoadingView:YES scrollView:nil success:^(id responseObject) {
    ...
    if (btn.tag == 1000)
    {
        [YGAlertView showAlertWithTitle:[NSString stringWithFormat:@"微信群号：%@\n加入微信群，一起沟通吧！",_model.groupNum] buttonTitlesArray:@[@"确定"] buttonColorsArray:@[colorWithMainColor] handler:^(NSInteger buttonIndex) {
...
{
    [YGAppTool shareWithShareUrl:_model.url shareTitle:_model.projectName shareDetail:_model.projectDescribe shareImageUrl:_model.picture shareController:self];
}
