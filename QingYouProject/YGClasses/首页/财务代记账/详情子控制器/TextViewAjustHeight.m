...
@interface TextViewAjustHeight ()<UITextViewDelegate>
/**
...
@implementation TextViewAjustHeight
...
- (void)setPlacehoderAligment:(NSTextAlignment)placehoderAligment
{
    _placehoderAligment = placehoderAligment;
    self.placeholderView.textAlignment = _placehoderAligment;
}
...
//        self.scrollEnabled = height > _maxTextH && _maxTextH > 0;
        if (height > _maxTextH && _maxTextH > 0) {
            [YGAppTool showToastWithText:@"字数达到上限"];
            return ;
        }
        _textH = height;
...
-(BOOL) textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if ([textView isFirstResponder]) {
        ...
        //判断键盘是不是九宫格键盘
        if ([YGAppTool isNineKeyBoard:text] ){
            return YES;
        }else{
            if ([YGAppTool hasEmoji:text] || [YGAppTool stringContainsEmoji:text]){
                return NO;
            }
        }
    }
