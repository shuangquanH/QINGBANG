//
//  MyDecorationApplyStatusController.m
//  QingYouProject
//
...
@interface MyDecorationApplyStatusController ()<UIScrollViewDelegate,YGSegmentViewDelegate>
...
@implementation MyDecorationApplyStatusController
...
- (void)addChildViewControllers{
    for (int i = 0; i < self.childArray.count; i++) {
        ...
        MyDecorationApplyOrderController * vc = [[className alloc] init];
        vc.statusString = [NSString stringWithFormat:@"%d",i];
        if ([vc.statusString isEqualToString:@"0"]) {
            vc.statusString = @"";
        }
