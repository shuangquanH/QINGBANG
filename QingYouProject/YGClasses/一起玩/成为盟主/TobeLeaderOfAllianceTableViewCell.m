//
//  TobeLeaderOfAllianceTableViewCell.m
//  QingYouProject
//
...
@implementation TobeLeaderOfAllianceTableViewCell
{
    UILabel *_nameLabel;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(10,0 , 120, 45)];
        _nameLabel.text = @"";
        ...
        if ([reuseIdentifier containsString:@"CrowdFundingAddProjectTableViewCelltextfield"]) {
            ...
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield1"]) {
                _textField.tag = 1002;
                _textField.placeholder = @"请选择性别";
            }
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield2"]) {
                _textField.tag = 1003;
                _textField.placeholder = @"请填写支付宝账号";
            }
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield3"]) {
                _textField.tag = 1004;
                _textField.placeholder = @"请填写电子邮箱";
            }
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield5"]) {
                _textField.tag = 1005;
                _textField.placeholder = @"请填写联盟名称";
            }
            //发布二手置换认证
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield6"]) {
                _textField.tag = 1005;
                _textField.placeholder = @"请填写联盟名称";
            }
            if ([reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelltextfield7"]) {
                _textField.tag = 1005;
                _textField.placeholder = @"请填写联盟名称";
            }
        }else
...
- (void)setModel:(CrowdFundingAddProjectModel *)model withIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.reuseIdentifier isEqualToString:@"CrowdFundingAddProjectTableViewCelllabel"]) {
