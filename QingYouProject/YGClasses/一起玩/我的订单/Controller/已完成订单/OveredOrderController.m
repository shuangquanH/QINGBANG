...
@interface OveredOrderController ()<UITableViewDelegate,UITableViewDataSource,OrderFooterViewDelegate>

@end

@implementation OveredOrderController
{
...
{
    [YGNetService YGPOST:REQUEST_ActivityOrderList parameters:@{@"userID":YGSingletonMarco.user.userId,@"type":@"3",@"total":self.totalString,@"count":self.countString} showLoadingView:NO scrollView:_tableView success:^(id responseObject) {
        if (headerAction == YES) {
