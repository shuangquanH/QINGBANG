//
//  AwaitingParticipationController.m
//  FrienDo
//
...
@interface AwaitingParticipationController ()<UITableViewDelegate,UITableViewDataSource>

@end

@implementation AwaitingParticipationController
{
...
{
    [YGNetService YGPOST:REQUEST_ActivityOrderList parameters:@{@"userID":YGSingletonMarco.user.userId,@"type":@"2",@"total":self.totalString,@"count":self.countString} showLoadingView:NO scrollView:_tableView success:^(id responseObject) {
        if (headerAction == YES) {
...
- (void)cancelActiveButtonClickedWithOrderFooterView:(OrderFooterView *)orderFooterView
{
    ...
    [YGAlertView showAlertWithTitle:@"确认要取消活动吗？" buttonTitlesArray:@[@"取消",@"确定"] buttonColorsArray:@[colorWithBlack,colorWithMainColor] handler:^(NSInteger buttonIndex) {
        ...
        {
            [YGNetService YGPOST:REQUEST_AllianceOrderCancel parameters:@{@"orderID":model.orderID} showLoadingView:NO scrollView:nil success:^(id responseObject) {
