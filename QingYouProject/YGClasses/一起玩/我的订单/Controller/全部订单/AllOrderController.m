...
@interface AllOrderController ()<UITableViewDelegate,UITableViewDataSource,OrderFooterViewDelegate>
...
{
    [YGNetService YGPOST:REQUEST_ActivityOrderList parameters:@{@"userID":YGSingletonMarco.user.userId,@"type":@"0",@"total":self.totalString,@"count":self.countString} showLoadingView:NO scrollView:_tableView success:^(id responseObject) {
        if (headerAction == YES) {
