...
#import "UIControl+SQSound.h"
#import <AVFoundation/AVFoundation.h>
#import <objc/runtime.h>

// Key for the dictionary of sounds for control events.
static char const * const sq_kSoundsKey = "sq_kSoundsKey";

@implementation UIControl (SQSound)
...
- (void)sq_setSoundNamed:(NSString *)name forControlEvent:(UIControlEvents)controlEvent
{
    ...
    AVAudioPlayer *oldSound = [self sq_sounds][oldSoundKey];
    ...
    NSMutableDictionary *sounds = [self sq_sounds];
...
- (void)setSQ_sounds:(NSMutableDictionary *)sounds
{
    objc_setAssociatedObject(self, sq_kSoundsKey, sounds, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSMutableDictionary *)sq_sounds
{
    NSMutableDictionary *sounds = objc_getAssociatedObject(self, sq_kSoundsKey);
    
    // If sounds is not yet created, create it.
    if (!sounds) {
        sounds = [[NSMutableDictionary alloc] initWithCapacity:2];
        // Save it for later.
        [self setSQ_sounds:sounds];
    }
