...
- (void)animWithUserID:(NSString *)userID model:(GiftModel *)model finishedBlock:(void(^)(BOOL result))finishedBlock {
    ...
    if ([self.userGigtInfos objectForKey:userID]) {
        ...
        if (self.queue3.operations.count <= self.queue2.operations.count && self.queue3.operations.count <=self.queue1.operations.count)
        {
            if (op.model.giftCount != 0) {
                op.presentView.frame  = CGRectMake(-self.parentView.frame.size.width / 2 , 180, self.parentView.frame.size.width / 2, 40);
                op.presentView.originFrame = op.presentView.frame;
                op.index = 3;
                [self.queue3 addOperation:op];
            }
        }
        else if(self.queue2.operations.count <= self.queue3.operations.count && self.queue2.operations.count <=self.queue1.operations.count)
        {
            if (op.model.giftCount != 0) {
        ...
        else if(self.queue1.operations.count <= self.queue3.operations.count && self.queue1.operations.count <= self.queue2.operations.count)
        {
            if (op.model.giftCount != 0) {
